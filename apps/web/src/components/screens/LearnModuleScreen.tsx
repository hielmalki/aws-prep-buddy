...
export function LearnModuleScreen({ module: mod, dark = false }: LearnModuleScreenProps) {
  const t = theme(dark);
  const router = useRouter();
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '18px 20px 120px' }}>
        ...
        {mod.slug === 'compute' ? (
          ...
          <div>
            ...
            <div style={{ marginTop: 20, padding: 16, borderRadius: 18, background: t.surface, border: `0.5px solid ${t.border}`, display: 'flex', alignItems: 'center', gap: 12 }}>
