...
import { useProgressStore, wrongQuestionNumbersInExam, gradeAnswer } from '@aws-prep/core';
import { getQuestion } from '@/lib/data';
import { theme, baseFont, mono, slate700, slate200 } from '@/lib/theme';
import { ProgressBar } from '@/components/ui/ProgressBar';
import { Chip } from '@/components/ui/Chip';
import { BottomNav } from '@/components/ui/BottomNav';
import { Close, Chevron, ChevronDown, Check, X } from '@/components/icons';
...
export function ReviewScreen({ examId, total, startIndex, dark = true }: ReviewScreenProps) {
  ...
  // Snapshot wrong list once on mount so it stays stable during review
  const [wrongNums] = useState<number[]>(() =>
    hydrated ? wrongQuestionNumbersInExam(answers, examId, total) : []
  );

  const [currentIdx, setCurrentIdx] = useState(Math.min(startIndex, Math.max(0, wrongNums.length - 1)));
  ...
  if (wrongNums.length === 0) {
    router.replace(`/quiz/result?exam=${examId}`);
    return null;
  }

  const questionNum = wrongNums[currentIdx];
  ...
  const goNext = () => {
    if (currentIdx + 1 < wrongNums.length) {
  ...
  return (
    <div style={{ background: t.bg, height: '100dvh', display: 'flex', flexDirection: 'column', fontFamily: baseFont, color: t.text, position: 'relative' }}>
      ...
      <div style={{ padding: '60px 20px 0', display: 'flex', alignItems: 'center', gap: 12 }}>
        <button onClick={() => router.push(`/quiz/result?exam=${examId}`)} style={{ width: 36, height: 36, borderRadius: 10, border: `1px solid ${t.border}`, background: t.surface, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, fontWeight: 600, color: t.textMuted, marginBottom: 6 }}>
            <span>Review {currentIdx + 1} / {wrongNums.length}</span>
            <span style={{ fontSize: 11 }}>Exam {examId}</span>
          </div>
          <ProgressBar pct={((currentIdx + 1) / wrongNums.length) * 100} t={t} />
        </div>
      </div>

      {/* content */}
      <div style={{ flex: 1, overflow: 'auto', padding: '22px 20px 160px' }}>
        ...
        {submitted && (
          <div style={{ marginTop: 16, borderRadius: 16, background: t.surface, border: `1px solid ${t.border}`, overflow: 'hidden' }}>
            <button onClick={() => setExpOpen(!expOpen)} style={{ width: '100%', padding: 14, border: 'none', background: 'transparent', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', color: t.text, fontFamily: baseFont }}>
              <div style={{ width: 28, height: 28, borderRadius: 8, background: gradeAnswer(question, picked) ? (dark ? 'rgba(74,222,128,0.14)' : '#F0FDF4') : (dark ? 'rgba(248,113,113,0.14)' : '#FEF2F2'), display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {gradeAnswer(question, picked) ? <Check size={14} color={t.green} /> : <X size={14} color={t.red} />}
              </div>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, padding: '12px 20px 24px', background: t.navBg, backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderTop: `1px solid ${t.border}`, zIndex: 35 }}>
        {!submitted ? (
          ...
          <button onClick={goNext} style={{ width: '100%', height: 52, borderRadius: 14, border: 'none', background: t.text, color: t.bg, fontSize: 15, fontWeight: 700, fontFamily: baseFont, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
            {currentIdx + 1 < wrongNums.length ? <>Next <Chevron size={18} color={t.bg} /></> : 'Back to results →'}
          </button>
        )}
      </div>
