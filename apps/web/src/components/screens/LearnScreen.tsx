...
interface LearnScreenProps { dark: boolean; }
...
export function LearnScreen({ dark }: LearnScreenProps) {
  const t = theme(dark);
  ...
  return (
    <div style={{ background: t.bg, height: '100dvh', display: 'flex', flexDirection: 'column', fontFamily: baseFont, color: t.text, position: 'relative' }}>
      ...
      <div style={{ padding: '60px 20px 0', display: 'flex', alignItems: 'center', gap: 10 }}>
        <button onClick={() => router.push('/')} style={{ width: 36, height: 36, borderRadius: 10, border: `1px solid ${t.border}`, background: t.surface, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transform: 'scaleX(-1)' }}>
          <Chevron size={18} color={t.text}/>
        </button>
        <div style={{ fontSize: 12, color: t.textMuted, fontWeight: 600 }}>Compute · Chapter 3 / 12</div>
      </div>
